...
class EnumeratedRecommendEnv(object):
    ...
    def __load_rl_data__(self, data_name, mode):
        if mode == 'train':
            with open(os.path.join(RAW_DATA_DIR[data_name], 'UI_Interaction_data/review_dict_valid.json'), encoding='utf-8') as f:
                print('train_data: load RL valid datasets')
                mydict = json.load(f)
        elif mode == 'test':
            with open(os.path.join(RAW_DATA_DIR[data_name], 'UI_Interaction_data/review_dict_test.json'), encoding='utf-8') as f:
                mydict = json.load(f)
        return mydict
